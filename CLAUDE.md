...
- **post-deployment** - Post-release cleanup after a deployment goes live: deletes local working branch, pushes master + aligns uat, creates and pushes the release tag, updates the Combinate Deployment Calendar sheet, posts the "Released in production" comment to Teamwork, and reassigns the task to the Support team. Trigger on "post deployment", "post deploy", "released in production", "deployment is live". Skill: `combinate-plugins/skills/01-General/post-deployment/SKILL.md`
