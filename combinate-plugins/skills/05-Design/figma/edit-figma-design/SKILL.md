...
**Use broader agent context selectively** when the user wants the design to match an existing product or design system:

- read project rules from `CLAUDE.md` if it exists
- use codebase search to find relevant components, product vocabulary, layout patterns, and design-token sources
- use other sources only when the prompt directly depends on them, such as product requirements or explicit inspiration requests
- do not edit code as part of this skill's normal workflow
...
- summarise what you created or updated at a high level
- ask whether the user wants revisions in Figma

If the user asks to implement the approved design in code, stop using this skill and consult `figma-implement-design`.
...
3. Read `CLAUDE.md` or search the codebase only if needed to understand billing terminology and existing components.
...
4. Return the same Figma URL and summarise the change.
...
Ask the user which team or organisation to use. Do not guess.

### Issue: The user wants the design to match existing product conventions, but the request is vague

Read `CLAUDE.md` first. Then use targeted codebase search to gather only the components and conventions relevant to the requested surface.

### Issue: The user asks for both a Figma design and implementation

Create or update the Figma design first only if the user's request is primarily about authoring in Figma. If the request is primarily about implementation, consult `figma-implement-design` instead. After the design is approved, implementation can follow in a separate step.
